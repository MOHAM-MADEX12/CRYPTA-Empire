...
local Window=Library.CreateLib("ðŸ”´ CRYPTA EMPIRE v5.7 CANDY EGG","BloodTheme")-- Changed to BloodTheme
local Watermark=Library:CreateWatermark("ðŸ”´ REDZ HUB | CRYPTA v5.7 CANDY EGG | MOHAM-MADEX12")
...
-- ðŸŒ¾ FARM
local Farm=Window:NewTab("ðŸŒ¾ Farm")
    ...
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")then
        ...
        if a then
            ...
            RunService.Heartbeat:Connect(function()
                if f then
                    local v=Camera.CFrame.LookVector*100
                    bv.Velocity=Vector3.new(v.X,UserInputService:IsKeyDown(Enum.KeyCode.Space)and b or UserInputService:IsKeyDown(Enum.KeyCode.LeftShift)and-b or 0,v.Z)
                end
            end)
        else bv:Destroy()bav:Destroy()end
    end
end)
Combat:NewSlider("Fly Speed","100",100,1,function(s)b=s end)
...
print("ðŸ”´ CRYPTA v5.7 CANDY EGG LOADED!")
